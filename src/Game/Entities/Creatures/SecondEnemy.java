 ...
 */
public class SecondEnemy extends CreatureBase  {
	...
	public SecondEnemy(Handler handler, float x, float y) {
        ...
        Secondinventory= new Inventory(handler);
    }
    ...
    public void tick() {
        ...
        if(handler.getWorld().equals(World1.caveWorld) && !isVisible() ){
        	setVisible(true);
        	this.x= 1250;
        	this.y= 300;
    ...
    protected void checkIfMove() {
        ...
        }	else if (SkelyCam.contains(handler.getWorld().getEntityManager().getPlayer().getX() - handler.getGameCamera().getxOffset(), handler.getWorld().getEntityManager().getPlayer().getY() - handler.getGameCamera().getyOffset())
